...
par(mar = c(0, 0, 1, 0 ))
plot(E_Af_No_b) #plot first to set up boundaries

col <- terrain.colors(6)
plot(msk_DEM, col = col,  alpha = 0.60, add = TRUE)


#TO ZOOM TO SPECIFIC REGION

#ext An extent object to zoom in a region (see also zoom and crop(x,drawExtent())

#-----------------------------------------------------------------------------
#Using ggplot  ----not run
# ggplot() +
#   geom_path(data = E_Af_No_b, aes(x = long, y = lat, group = group)) +
#   labs(title = "East aFRICA - using ggplot") 
...
orange <- rbind(st_as_sf(KEN2_5to10) %>% select(NAME_1), 
              st_as_sf(KEN3_5to10) %>% select(NAME_1))
plot(st_geometry(st_centroid(orange )), pch = 20, col = "orange ", add = TRUE)
