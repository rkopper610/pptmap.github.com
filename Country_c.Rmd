...
```{r}
t2 <- t + theme_bw() + facet_grid(labeller = label_value(multi_line = TRUE), cols = vars(date_cat), space = "free_x", scales = "free_x") +
  theme(axis.text.x = element_text(size = 10, angle = 90, vjust = .5 ,  hjust = 1), legend.position = "bottom") + ylab("Number of studies") + scale_fill_manual(name = "Type of study", labels = c("field experiment", "farmer survey") , values = c("#0072B2", "#D55E00")) 
